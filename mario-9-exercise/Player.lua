...
function Player:update(dt)
    ...
    if self.dy < 0 then
        -- TODO: if any two tiles right above us are not empty, reset our Y
        -- velocity, then check whether they're question blocks and change them
        -- to a dark version if they are
        -- HINT: be sure to check both tiles right above you by using your X
        -- coordinate and your X coordinate + your width!
        -- HINT: self.map:tileAt should come in handy!
        -- don't forget to call self.map:refreshSpriteBatch if we change a tile!
        -- HINT: self.map:setTile will come in handy
        -- HINT: math.floor will come in handy so we don't try and accidentally
        -- set a floating-point index in our map!
        -- HINT:
    end
